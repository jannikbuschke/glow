...
type StringAndGuidDep = { Id: System.Guid; Name: string }
...
type ListIntRecord = { List: int list }
...
type SeqTypeRecord = { List: MyType seq }
...
type TransitiveGenericRecord = { Result: Result<int, string option> }
...
type Request =
  { Id: string }
  interface IRequest<int list>

[<Fact>]
let ``Record with interface`` () =
  let deps = Glow.SecondApproach.getDependencies typeof<Request>
  deps |> should haveLength 3
  deps |> should contain typeof<int>
  deps |> should contain typedefof<list<_>>
  deps |> should contain typeof<string>

type EventViewmodel = { Version: int64 }
