module Glow.Core.TsGen.GenerateApi2
...
let render (assemblies: Assembly list) (path: string) =
  ...
  let getTypeName (t: System.Type) =
    let moduleName = Glow.SecondApproach.getModuleName t
    let name = Glow.SecondApproach.getPropertySignature "" t
    name
