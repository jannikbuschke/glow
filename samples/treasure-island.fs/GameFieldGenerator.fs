...
module GameFieldGenerator =
  ...
  let orientedRectangle2 height width =
    let random = Random()
    ...
    for q in 1..width do
      ...
      for r in -offset .. (height - offset) do
        ...
        let field =
          { Field.Items = []
            Field.Tile = material
            Field.Position = { Q = q; R = r; S = -q - r } }

        hexas.Add(field) // Field.New(new Position(q, r, -q - r), material))
    ...
    let result: GameField =
      { Fields = hexas |> Seq.toList }

    result

  let hexagon mapRadius =
    let random = Random()
    let hexas = List<Field>()

    let pickRandomMaterial = true

    for q in -mapRadius .. (mapRadius + 1) do
      let r1 =
        Math.Max(-mapRadius, -q - mapRadius)

      let r2 = Math.Min(mapRadius, -q + mapRadius)

      for r in r1 .. (r2 + 1) do
        let material =
          if pickRandomMaterial then
            tiles.[random.Next(tiles.Length)]
          else
            tiles.[(q + r) % tiles.Length]
